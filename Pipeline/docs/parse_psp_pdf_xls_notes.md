...
The grid operator publishes each day's report as a PDF or an Excel file, formatted for a human to read, not a computer. This is the translator: it opens that file, finds the actual numbers (how much electricity was used, how much was generated by coal vs. solar vs. hydro, whether any power plants were down, etc.), and turns them into a clean row of data. This is the single most important piece of code in the whole project — every other dataset either comes from this parser directly or depends on it indirectly.
