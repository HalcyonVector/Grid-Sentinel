...
**Frequency-violation classifier** (`03_violation_baseline.ipynb`, LightGBM, time-aware split 2024-11→2025-06 train / 2025-07→2025-12 val / 2026 test): **PR-AUC 0.0937** vs. a 0.0305 base-rate baseline (~3x lift). F1 at a naive 0.5 threshold is 0.0207 (still near-zero — 0.5 is still the wrong lens for a ~3% positive rate); at the best-F1 operating point (threshold ≈0.09): **F1 0.1712, precision 12.1%, recall 29.0%**. Recall at ≥95% precision is still effectively 0 — a high-confidence-only alert mode isn't achievable with this feature set yet. **Reported as a weak-but-real baseline, not oversold.**
...
**Ramp-shock classifier** (`04_ramp_shock_baseline.ipynb`, same split/pipeline): **PR-AUC 0.7248** vs. a 0.1793 base-rate baseline (~4x lift) — a genuinely strong signal, and slightly stronger still after the same two fixes described above (was PR-AUC 0.7140). F1@0.5 dropped to 0.5436 (was 0.6293) as an expected side effect of no longer using `scale_pos_weight` to artificially recalibrate probabilities around the 0.5 boundary — the threshold-optimized number is what actually matters here: best-F1 0.6570 (was 0.6293), precision 63.4%, recall 68.1%. Recall at ≥95% precision = 22.4% (was 17.7%). Top features: `hour`, `demand_delta_mw`, and now `solar_delta_mw`/`solar_roll8_std` (3rd and 4th, ahead of most demand-lag features) — consistent with the EDA's sunrise/sunset clustering. Corridor columns place in the top 15 but are secondary to the time/demand/solar signal, consistent with Era 2's finding that corridor flow has a real but non-dominant relationship with stress.
...
**Lead-window experiment, 2026-07-11** (appended to `03_violation_baseline.ipynb` as a second cell, re-run after the `scale_pos_weight` fix above): does shrinking the violation target's lookahead from 1-4 slots help? `features.py`'s `add_violation_label()` and `build_feature_table()` were parameterized (`lead_slots` / `violation_lead_slots`, default unchanged at 4) to test this without disturbing the shipped baseline or the ramp-shock target.

| lead_slots | base rate | PR-AUC | lift over base rate | best-F1 | precision | recall |
|---|---|---|---|---|---|---|
| 4 (shipped, 15-60 min) | 3.05% | 0.0937 | 3.07x | 0.1712 | 12.1% | 29.0% |
| 3 (15-45 min) | 2.46% | 0.0946 | 3.84x | 0.1649 | 12.0% | 26.6% |
| 2 (15-30 min) | 1.81% | 0.1254 | 6.92x | 0.1849 | 24.1% | 15.0% |
| 1 (15 min only) | 1.11% | 0.0985 | 8.88x | 0.1687 | 12.9% | 24.5% |

**Different conclusion than the first pass** (which used the buggy `scale_pos_weight` config and understated every window). This is no longer a flat trade-off with no winner: **2 slots now has both the highest PR-AUC (0.1254) and the highest best-F1 (0.1849) of all four windows** — a materially stronger case than before. The trade-off that remains is precision vs. recall specifically at that window: 2 slots gets much higher precision (24.1% vs. the shipped 4-slot's 12.1%) at the cost of recall (15.0% vs. 29.0% — catching fewer total violations). Whether to actually *ship* a shorter window is still a product decision (fewer, higher-confidence 30-min warnings vs. more, noisier 60-min ones), not a pure modelling one, so **the shipped default remains 4 slots** — but the case for revisiting that default is now considerably stronger than the original (bug-affected) experiment suggested, and worth an explicit decision rather than leaving it on the original scope by default.
...
6. ~~**`03_violation_baseline.ipynb`**~~ ✅ Done — PR-AUC 0.0937 (corrected 2026-07-11, see below).
7. ~~**`04_ramp_shock_baseline.ipynb`**~~ ✅ Done — PR-AUC 0.7248 (corrected 2026-07-11, see below).
