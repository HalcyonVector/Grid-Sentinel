...
_Last updated: 2026-07-11 (night) — did a real confidence audit of Phase 4 rather than just re-reading the docs (Sagnik asked how much confidence we actually have). Found that `share_res_pct` and all 11 `ir_*`/`xb_*` corridor columns are whole-DAY aggregates broadcast identically to every 15-min slot of that day (verified directly), which raised a genuine question: does that leak later-in-day information into an earlier slot's prediction? Tested rather than assumed — removing them from the classifiers' feature set **improved both targets** (violation PR-AUC 0.0937 → 0.1186, ramp-shock 0.7248 → 0.7446), so there was no leakage inflation, just noise being removed. Re-ran the lead-window experiment a third time on the new feature set; the "best" window changed again (see Phase 4), which itself is now the headline finding of that experiment — the ranking isn't stable enough across passes to trust any single one. All three baseline notebooks, `features.py`, and `predict.py` updated and re-verified against a real Colab-equivalent setup._
...
**Frequency-violation classifier** (`03_violation_baseline.ipynb`, LightGBM, time-aware split 2024-11→2025-06 train / 2025-07→2025-12 val / 2026 test): **PR-AUC 0.1186** vs. a 0.0305 base-rate baseline (~3.9x lift). F1 at a naive 0.5 threshold is near-zero (0.5 is the wrong lens for a ~3% positive rate); at the best-F1 operating point (threshold ≈0.06): **F1 0.1852, precision 13.6%, recall 29.0%**. Recall at ≥95% precision is 0.0036 — effectively zero, not a usable high-confidence alert mode. **Reported as a weak-but-real baseline, not oversold.**
...
**Ramp-shock classifier** (`04_ramp_shock_baseline.ipynb`, same split/pipeline): **PR-AUC 0.7446** vs. a 0.1793 base-rate baseline (~4.2x lift) — a genuinely strong signal, and stronger still after the fixes described above (was PR-AUC 0.7140 → 0.7248 → 0.7446 across three rounds of real fixes). Best-F1 0.6785 (was 0.6293 → 0.6570), precision 62.8%, recall 73.8%. Recall at ≥95% precision = 19.7% — this is the one metric that went slightly down from the previous round (was 22.4%), reported honestly rather than only highlighting what improved. Top features: `hour`, `demand_delta_mw`, `month`, `solar_delta_mw`, `demand_met_mw_lag3` — solar and demand-trajectory features dominate, consistent with the EDA's sunrise/sunset clustering. Corridor columns no longer appear at all (removed from `FEATURE_COLS`, see above) — Era 2's daily-resolution corridor-flow finding remains a separate, valid, unaffected result, it just isn't what drives this live classifier.
...
**Lead-window experiment — run a THIRD time, 2026-07-11**, each time on a meaningfully different (and better) feature set. `features.py`'s `add_violation_label()` and `build_feature_table()` are parameterized (`lead_slots` / `violation_lead_slots`, default unchanged at 4) to test this without disturbing the shipped baseline or the ramp-shock target.

| lead_slots | base rate | PR-AUC | lift over base rate | best-F1 | precision | recall |
|---|---|---|---|---|---|---|
| 4 (shipped, 15-60 min) | 3.05% | 0.1186 | 3.88x | 0.1852 | 13.6% | 29.0% |
| 3 (15-45 min) | 2.46% | 0.0951 | 3.86x | 0.1609 | 11.9% | 24.8% |
| 2 (15-30 min) | 1.81% | 0.1329 | 7.33x | 0.1767 | 17.1% | 18.4% |
| 1 (15 min only) | 1.11% | 0.1129 | 10.18x | 0.2228 | 24.4% | 20.5% |

**Worth being explicit about the pattern across all three passes, not just this latest table:** first pass (buggy `scale_pos_weight`) found no clean winner; second pass found 2 slots clearly best on every metric; this third pass finds 1 slot has the best F1 while 2 slots still has the best PR-AUC and 4 slots remains competitive on PR-AUC too. The ranking has moved every time the feature set changed — that instability is itself the finding: whatever the "optimal" window is, it isn't robust enough to treat any single run's ranking as a final answer, and picking a "winner" here would need more robust validation (repeated splits, cross-validation) than a single time-aware split provides. The one *stable* pattern across all three passes: shorter windows trade recall for precision/F1, longer windows trade precision for recall and total events caught. **The shipped default remains 4 slots** (matching the roadmap's original 1-4-slot scope) — not because it's been shown best, but because no window has been shown robustly best, and changing the product's core lead-time promise deserves more than one unstable experiment behind it.
...
6. ~~**`03_violation_baseline.ipynb`**~~ ✅ Done — PR-AUC 0.1186 (corrected 2026-07-11, three rounds of real fixes, see below).
7. ~~**`04_ramp_shock_baseline.ipynb`**~~ ✅ Done — PR-AUC 0.7446 (corrected 2026-07-11, three rounds of real fixes, see below).
...
**Old framing (superseded):** "shows, for the first time, that inter-regional corridor congestion and cross-border exchange have measurable predictive value for short-term grid stress" — this leads with the weakest piece of evidence (the violation classifier, PR-AUC 0.1186 as of the latest fix) and a cross-border/corridor result that, as of 2026-07-11, turned out not to hold up as a *live-classifier* feature at all (see Phase 4's `FEATURE_COLS` note — corridor columns were removed from the classifiers after being found to hurt rather than help). Doesn't reflect what the project actually found strongest.
